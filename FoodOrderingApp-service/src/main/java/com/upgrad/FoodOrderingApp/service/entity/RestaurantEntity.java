...
@NamedQueries({
        ...
        @NamedQuery(
                ...
                query =
                        "select r from RestaurantEntity r where lower(r.restaurantName) like lower(:searchString) "
                                + "order by r.restaurantName asc"),
        @NamedQuery(
                name = "restaurantByCategory",
                query =
                        "Select r from RestaurantEntity r where r.id in (select rc.restaurantId from RestaurantCategoryEntity rc where rc.categoryId = "
                                + "(select c.id from CategoryEntity c where "
                                + "c.uuid=:categoryUuid) ) order by r.restaurantName")
})
